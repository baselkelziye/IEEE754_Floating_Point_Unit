...
from helpers import generate_add_case, generate_sub_case, generate_multiply_case, generate_divide_case, FPUoperation  # Assuming your helper script is named `helper.py`
import struct
...
async def test_fpu_adder(dut):
    ...
    """
    for i in range(TESTNUM):  # Generate and test 100 random cases
        # Generate test case
...
@cocotb.test()
#write test_fpu_divider using the same pattern above
async def test_fpu_divider(dut):
    """
    Test the FPU divider module for correctness with random cases.
    """
    for i in range(TESTNUM):
        # Generate test case
        a, b, expected_result, op = generate_divide_case()

        # Apply inputs
        dut.num1.value = int(a.hex, 16)
        dut.num2.value = int(b.hex, 16)
        dut.op.value = op.value
        ...
        if abs(error_magnitude) > 0.1:
            dut._log.error(
            f"Test failed for inputs {a.float} / {b.float}:\n"
            f"a = (0x{a.hex}), b = (0x{b.hex})\n"
            f"Expected: {expected_float} (0x{expected_result.hex})\n"
            f"Got: {dut_res_float} (0x{int(result):08x})"
)
